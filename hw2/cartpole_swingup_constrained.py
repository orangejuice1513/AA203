...
def scp_iteration(f, s0, s_goal, s_prev, u_prev, N, P, Q, R, u_max, ρ):
    ...
    for k in range(N):      
        stage_cost = cvx.quad_form(s_cvx[k] - s_goal, Q) + cvx.quad_form(u_cvx[k], R)
        objective += stage_cost 

        constraints.append(s_cvx[k+1] == A[k] @ s_cvx[k] + B[k] @ u_cvx[k] + c[k]) # linear dynamics 
        constraints.append(cvx.abs(u_cvx[k]) <= u_max) # control bounds 

        constraints.append(cvx.norm(s_cvx[k] - s_prev[k], "inf") <= ρ)# state trust region 
        constraints.append(cvx.norm(u_cvx[k] - u_prev[k], "inf") <= ρ) # control trust region

    objective += cvx.quad_form(s_cvx[N] - s_goal, P) # add terminal cost 
    constraints.append(cvx.norm(s_cvx[N] - s_prev[N], "inf") <= ρ) # add terminal state trust region
