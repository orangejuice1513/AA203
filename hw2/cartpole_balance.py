...
def compute_lti_matrices() -> tuple[np.ndarray, np.ndarray]:
    ...
    [1.0, 0.0, dt, 0.0],
    [0.0, 1.0, 0.0, dt],
    [0.0, dt * (mp * g) / mc, 1.0, 0.0],
    [0.0, dt * (mc + mp) * g / (mc * L), 0.0, 1.0]
    ])
    B = B = np.array([
    [0.0],
    [0.0],
...
def main():
    ...
    def reference(ti: float) -> np.ndarray:  
        # new s_ref calculation from pt d  
        a = 10.0 
        T = 10.0 
        return np.array([ 
            a * np.sin(2*np.pi*ti/T), 
            np.pi, 
            (2*np.pi*a/T) * np.cos(2*np.pi*ti/T), 
            0.0, 
        ]) 
