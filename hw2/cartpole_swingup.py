...
def ilqr(f, s0, s_goal, N, Q, R, QN, eps=1e-3, max_iters=1000):
    ...
    for _ in range(max_iters):
        ...
        for k in range(N - 1, -1, -1):
            ...
            Y[k] = -np.linalg.solve(Q_uu, Q_ux)
            y[k] = -np.linalg.solve(Q_uu, Q_u)
        ...
        # step forward from 0 to N-1
        for k in range(N):
            du[k] = Y[k] @ ds[k] + y[k]
            u_bar[k] = u_bar[k] + du[k]
            new_s = f(s_bar[k], u_bar[k])
            ds[k+1] = new_s - s_bar[k+1]
            
            s_bar[k+1] = new_s # update nominal state trajectory 
